...
class RabbitMQQueue extends Queue implements QueueContract
{
    ...
    {
        return $this->container->make(RabbitMQPayloadPackerInterface::class);
    }
    ...
    private function declareQueue($name)
    {
        ...
        if ($this->declareBindQueue && ! in_array($name, $this->declaredQueues, true)) {
            // declare queue
            $this->channel->queue_declare($name, $this->configQueue['passive'], $this->configQueue['durable'], $this->configQueue['exclusive'], $this->configQueue['auto_delete']);
    ...
    private function declareDelayedQueue($destination, $delay)
    {
        ...
        if (! in_array($name, $this->declaredQueues, true)) {
            $this->channel->queue_declare($name, $this->configQueue['passive'], $this->configQueue['durable'], $this->configQueue['exclusive'], $this->configQueue['auto_delete'], false, new AMQPTable([
